...
export class Book {
  ...
  constructor(
    ...
    // Create a deferred for the book hash so readiness can wait until
    // the hash has been generated for archived books. It will be
    // resolved in `unpack()` once the packaging/path and archive are
    // available (or immediately for non-archived books).
    this.loading!.hash = new defer<string | undefined>();
