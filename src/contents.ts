 ...
 */
class Contents {
  public onResize?: (size: { width: number; height: number }) => void;
  public epubcfi: EpubCFI = new EpubCFI();
  ...
  public expanding: NodeJS.Timeout | string | number | undefined = 0;
  private _expanding: boolean = false;
  public observer: ResizeObserver | null = null;
  public emit: (event: string, ...args: unknown[]) => void =
    EventEmitter.prototype.emit;

  private _layoutStyle: string | undefined;

  private _resizeCheck: () => void = this.resizeCheck.bind(this);
   ...
   */
  width(w: number | string | undefined) {
   ...
   */
  height(h: number | string | undefined) {
    const frame = this.content;
   ...
   * @returns {number} width
   */
  contentWidth(w: number | string | undefined) {
   ...
   * @returns {number} height
   */
  contentHeight(h: number | string | undefined) {
    const content = this.content || this.document.body;
   ...
   */
  overflowX(overflow: string | undefined) {
    if (overflow) {
   ...
   */
  overflowY(overflow: string | undefined) {
    if (overflow) {
   ...
   */
  css(property: string, value: string | undefined, priority?: boolean) {
    const content = this.content || this.document.body;
    ...
    // property should be castable to a number
   ...
   */
  viewport(options?: ViewportOptions | undefined) {
    // var width, height, scale, minimum, maximum, scalable;
    let $viewport = this.document.querySelector("meta[name='viewport']");
    const parsed: ViewportOptions = {
      width: undefined,
      height: undefined,
      scale: undefined,
      minimum: undefined,
    ...
    settings = defaults<ViewportOptions>(options || {}, parsed);
  ...
  locationOf(
    ...
    if (this.epubcfi.isCfiString(target)) {
      const range = new EpubCFI(target as string).toRange(
        this.document,
        ignoreClass
      );
   ...
   * @param {string} key If the key is the same, the CSS will be replaced instead of inserted
   */
  addStylesheetRules(rules: Array | object, key: string) {
  ...
  triggerSelectedEvent(selection) {
    ...
    if (selection && selection.rangeCount > 0) {
      ...
      console.log('[Contents] selection range', range);
      if (!range.collapsed) {
        // cfirange = this.section.cfiFromRange(range);
        cfirange = new EpubCFI(range, this.cfiBase).toString();
        console.log('[Contents] selection cfi', cfirange);
   ...
   * @returns {Range} range
   */
  range(_cfi: Range, ignoreClass?: string): Range | null {
   ...
   * @param {number} gap
   */
  columns(width: number, height: number, columnWidth: number, gap: number, dir: string) {
   ...
   * @param {number} height
   */
  fit(width: number, height: number, section: Section | undefined) {
    const viewport = this.viewport();
    if (!viewport.width || isNaN(Number(viewport.width))) {
      throw new Error('Viewport width is missing or invalid');
    }
    if (!viewport.height || isNaN(Number(viewport.height))) {
      throw new Error('Viewport height is missing or invalid');
    }
    const viewportWidth = parseInt(viewport.width, 10);
    const viewportHeight = parseInt(viewport.height, 10);
    if (!viewportWidth || !viewportHeight) {
      throw new Error('Viewport width or height is zero or invalid');
    }
   ...
   * @param {string} [mode] Any valid CSS writing-mode string (e.g. "horizontal-tb", "vertical-rl", "vertical-lr", etc.)
   */
  writingMode(mode: string | undefined) {
    const WRITING_MODE = prefixed('writing-mode');

    if (mode && this.documentElement) {
      // TypeScript: WRITING_MODE is string, so cast to string index
    ...
    const computed = this.window.getComputedStyle(this.documentElement);
    const value = ((computed as unknown) as Record<string, unknown>)[WRITING_MODE] as string;
