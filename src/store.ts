...
type Requester = {
  (url: string, type: string, withCredentials?: boolean, headers?: Record<string, string>): Promise<unknown>;
}

type Resolver = (href: string) => string;
 ...
 */
class Store {
  public urlCache: { [key: string]: string } = {};
  public storage: LocalForage | undefined;
  public name: string;
  public requester: Requester;
  public resolver: Resolver;
  public online: boolean = true;

  private _status: (() => void) | undefined;
  public emit: (event: string, ...args: unknown[]) => void =
    EventEmitter.prototype.emit;

  constructor(name: string, requester: Requester, resolver: Resolver) {
    this.name = name;
   ...
   * @return {Promise<Blob>}
   */
  put(url: string, withCredentials?: boolean, headers?: Record<string, string>): Promise<Blob> {
    const encodedUrl = window.encodeURIComponent(url);
   ...
   */
  handleResponse(response: unknown, type: string) {
    let r;
