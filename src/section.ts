...
import { HooksObject } from 'types/utils/hook';
import { LayoutSettings, Match } from 'types/section';
...
class Section {
  ...
  constructor(item: Section, hooks: HooksObject) {
  ...
  base() {
    if (!this.document) {
      throw new Error('Section document is not loaded');
    }
   ...
   * @param _query The query string to search
   * @param maxSeqEle The maximum number of Element that are combined for search, default value is 5.
   */
