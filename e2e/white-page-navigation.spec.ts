...
test.describe('EPUB Navigation - White Page Issue Prevention', () => {
  ...
  test('should handle rapid backward navigation without white pages', async ({
    page,
  }) => {
  ...
  test('should handle scroll position correctly during rapid navigation', async ({
    ...
    const problematicResets = events.filter(
      (event) => event.from > 0 && event.to === 0
    );
    ...
    for (let i = 0; i < 5; i++) {
      ...
      while (attempts < maxAttempts) {
        ...
        if (!duringState.hasVisibleContent) {
          console.warn(
            `Temporary white page during transition attempt ${attempts}`
          );
        }
      ...
      expect(
        afterState.hasVisibleContent,
        `Final state has no content. Transitioned from ${beforeState.chapter} to ${afterState.chapter}`
      ).toBe(true);
    }
  });

  test('should reproduce white page issue with extreme backward navigation', async ({
    page,
  }) => {
    // Use the exact same page from the logs
    await page.goto(
      'http://localhost:3000/examples/transparent-iframe-hightlights.html'
    );

    // Wait for the book to load completely
    await page.waitForFunction(
      () => (window as any).book && (window as any).rendition
    );
    await page.waitForTimeout(3000);

    console.log(
      '=== Reproducing white page issue with extreme backward navigation ==='
    );
    ...
    while (step < 15 && !detectedWhitePage) {
      // Try up to 15 backward steps
      step++;
      ...
      if (afterState.isWhitePage) {
        ...
        console.error(
          `Chapter: ${afterState.chapter}, Index: ${afterState.index}`
        );
        console.error(
          `ScrollLeft: ${afterState.scrollLeft}, ScrollWidth: ${afterState.scrollWidth}`
        );
        console.error(
          `Iframe content length: ${afterState.iframeContentLength}`
        );
        ...
        if (!delayedCheck.hasContentNow) {
          console.error(
            'WHITE PAGE BUG CONFIRMED - No content after 3 second delay'
          );
      ...
      if (
        beforeState.chapter === afterState.chapter &&
        beforeState.scrollLeft === afterState.scrollLeft
      ) {
        console.log(
          `Navigation seems stuck at ${afterState.chapter} - may have reached beginning`
        );
        break;
      }
    ...
    if (detectedWhitePage) {
      console.log(
        `=== WHITE PAGE ISSUE SUCCESSFULLY REPRODUCED ON STEP ${step} ===`
      );
    } else {
      console.log(
        '=== No white page issue detected in extreme backward navigation ==='
      );
    }
  });

  test('should fail when white page bug occurs - regression prevention test', async ({
    page,
  }) => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (state.isWhitePage) {
        whitePage = true;
        console.log(
          'WHITE PAGE BUG DETECTED - This test should fail until fixed'
        );
        break;
      }
    ...
    expect(
      whitePage,
      'White page bug was reproduced - this test should fail until the bug is fixed'
    ).toBe(false);
    expect(
      unknownChapter,
      'Unknown chapter detected - navigation state was corrupted'
    ).toBe(false);

    console.log(
      '=== Regression test completed - should have failed if bug exists ==='
    );
  });

  test('should debug sections before chapter 1 - investigating white page source', async ({
    page,
  }) => {
    await page.goto(
      'http://localhost:3000/examples/transparent-iframe-hightlights.html'
    ...
    const chapter1Index = allSections.findIndex((section: any) =>
      section.href.includes('chapter_001')
    );
    ...
    if (chapter1Index > 0) {
      ...
      for (let step = 0; step < chapter1Index + 2; step++) {
        ...
        if (!afterState.hasContent) {
          ...
          console.log(
            'This is likely a section before chapter 1:',
            beforeState.chapter,
            '->',
            afterState.chapter
          );
  ...
  test('should debug width calculations for sections causing white pages', async ({
    page,
  }) => {
    await page.goto(
      'http://localhost:3000/examples/transparent-iframe-hightlights.html'
    ...
    for (const section of testSections) {
      ...
      const info = await page.evaluate(() => {
        ...
        const phantomElement = viewer?.querySelector(
          '.epub-scroll-phantom'
        ) as HTMLElement;
      ...
      if (info.phantomExists && info.phantomWidth !== info.bodyScrollWidth) {
        console.log(
          `⚠️ ${section.name}: Width mismatch - phantom: ${info.phantomWidth}, body: ${info.bodyScrollWidth}`
        );
      }

      if (info.bodyScrollWidth > info.containerScrollWidth) {
        console.log(
          `⚠️ ${section.name}: Content wider than container - ${info.bodyScrollWidth} > ${info.containerScrollWidth}`
        );
      }
    }
