...
test.describe('Pre-rendered Object Inspection', () => {
  test('should inspect pre-rendered chapter objects', async ({ page }) => {
    ...
    await page.goto(
      'file://' + process.cwd() + '/examples/prerendering-example.html'
    );
          ...
          if (chapter) {
            results.chapters[href] = {
              ...
              element: {
                ...
                htmlPreview:
                  chapter.element?.innerHTML?.substring(0, 300) + '...',
              },
              ...
              view: chapter.view
                ? {
                    ...
                    element: {
                      ...
                      parentNodeType:
                        chapter.view.element?.parentNode?.nodeType,
                      parentNodeName:
                        chapter.view.element?.parentNode?.nodeName,
                    },
                    iframe: chapter.view.iframe
                      ? {
                          ...
                          contentDocument:
                            !!chapter.view.iframe.contentDocument,
                          readyState:
                            chapter.view.iframe.contentDocument?.readyState,
                        }
                      : { exists: false },
                    contents: chapter.view.contents
                      ? {
                          exists: true,
                          documentReady:
                            chapter.view.contents.document?.readyState,
                          textWidth:
                            typeof chapter.view.contents.textWidth ===
                            'function'
                              ? chapter.view.contents.textWidth()
                              : 'N/A',
                          scrollWidth:
                            typeof chapter.view.contents.scrollWidth ===
                            'function'
        ...
        if (testChapter) {
          ...
          if (attachedChapter) {
            ...
            try {
              ...
              if (
                attachedChapter.element &&
                attachedChapter.element.parentNode !== viewer
              ) {
    ...
    if (inspectionResults.debugInfo) {
      console.log(
        'Debug Info:',
        JSON.stringify(inspectionResults.debugInfo, null, 2)
      );
    }

    console.log('\n=== CHAPTER INSPECTION ===');
    for (const [href, chapterInfo] of Object.entries(
      inspectionResults.chapters
    )) {
      console.log(`\n--- ${href} ---`);
    ...
    expect(
      inspectionResults.preRendererExists,
      'Pre-renderer should exist'
    ).toBe(true);
    ...
    if (chapter1?.exists !== false) {
      expect(
        chapter1.pageCount,
        'Chapter 1 should have a page count'
      ).toBeGreaterThan(0);
      expect(chapter1.width, 'Chapter 1 should have width').toBeGreaterThan(0);
      expect(chapter1.height, 'Chapter 1 should have height').toBeGreaterThan(
        0
      );
      expect(chapter1.element.exists, 'Chapter 1 should have an element').toBe(
        true
      );
  ...
  test('should navigate next from chapter 1 and report location', async ({
    page,
  }) => {
    await page.goto(
      'file://' + process.cwd() + '/examples/prerendering-example.html'
    ...
    const result = await page.evaluate(async () => {
      const rendition =
        (window as any).getRendition?.() || (window as any).rendition;
      if (!rendition) {
  ...
  test('should cross boundary into chapter 1 from epigraph and land at page 1', async ({
    page,
  }) => {
    await page.goto(
      'file://' + process.cwd() + '/examples/prerendering-example.html'
    ...
    expect(
      result.crossed,
      'Should reach chapter_001.xhtml after next() hops'
    ).toBeTruthy();
