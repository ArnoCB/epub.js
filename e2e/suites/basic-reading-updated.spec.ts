...
test.describe('Basic EPUB Reading Tests', () => {
  test('can open and read an EPUB from URL (Moby Dick)', async ({ page, baseURL }) => {
    ...
    try {
      ...
      // Load Moby Dick directly in the page
      const bookStatus = await page.evaluate(async () => {
        try {
          // Clean approach that matches epub.spec.ts
          // Create the book object
          const book = (window as any).ePub('https://s3.amazonaws.com/moby-dick/OPS/package.opf');
      ...
      console.log('Moby Dick loading status:', bookStatus);
      ...
      if (bookStatus.success) {
        ...
        if (bookStatus.error && 
          ...
          console.log(`Network error loading Moby Dick: ${bookStatus.error}`);
          test.skip();
        } else {
          // Otherwise, mark test as failed
          console.log(`Failed to load Moby Dick: ${bookStatus.error}`);
          expect(bookStatus.success).toBe(true);
        }
      }
      
      // Take a screenshot for diagnostic purposes
      await page.screenshot({ path: 'test-results/moby-dick-final.png' });
    } catch (error) {
