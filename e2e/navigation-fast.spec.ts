...
const TEST_CONFIG = {
  ...
  REMOTE_MOBY: {
    ...
    isLocal: false,
    expectedChapters: ['chapter_001.xhtml', 'chapter_002.xhtml', 'chapter_003.xhtml']
  }
};

// ===== OPTIMIZED HELPERS =====
const setupEpub = async (page: any, epubUrl: string, spreadMode: boolean = true) => {
  // Use the correct base URL from our Playwright config
  await page.goto('/examples/spreads.html');
  
  // Wait for EPUB.js to be loaded
  await page.waitForFunction(() => typeof (window as any).ePub === 'function', { timeout: 10000 });
  
  await page.evaluate(({ url, spread }) => {
  ...
  await page.waitForFunction(() => {
    ...
    return rendition && rendition.manager && rendition.manager.views.length > 0;
  }, { timeout: 15000 });
...
const performBatchNavigation = async (page: any, steps: string[]): Promise<NavigationResult[]> => {
...
test.describe('EPUB Navigation - Core Features (Parallel)', () => {
  ...
  test('LOCAL Alice EPUB - Single Page Mode', async ({ page }) => {
    ...
    const navResults = await performBatchNavigation(page, ['next', 'next', 'prev', 'next']);
    ...
    navResults.forEach((result, i) => {
      ...
      console.log(`📖 Alice Single ${i+1}: ${result.chapter} pages=[${result.pages}] scroll=${result.scrollLeft}/${result.scrollWidth}`);
    });
    ...
    console.log(`✅ LOCAL Alice Single: ${navResults.length} navigations, ${uniqueChapters.length} chapters`);
  });
  
  test('LOCAL Alice EPUB - Spread Mode', async ({ page }) => {
    await setupEpub(page, TEST_CONFIG.LOCAL_ALICE.url, true);
    
    const navResults = await performBatchNavigation(page, ['next', 'next', 'prev', 'next', 'next']);
    
    navResults.forEach((result, i) => {
      expect(result.content).toBe(true);
      expect(result.spread).toBe(true); // Spread mode
      console.log(`📖 Alice Spread ${i+1}: ${result.chapter} pages=[${result.pages}] spread=true`);
    });
    
    console.log(`✅ LOCAL Alice Spread: ${navResults.length} navigations completed`);
  });
  ...
  test('REMOTE Moby Dick EPUB - Spread Mode', async ({ page }) => {
    ...
    const forwardResults = await performBatchNavigation(page, ['next', 'next', 'next', 'next', 'next', 'next', 'next', 'next']);
    const backwardResults = await performBatchNavigation(page, ['prev', 'prev', 'prev']);
    ...
    backwardResults.forEach((result, i) => {
      expect(result.content).toBe(true);
      console.log(`🔙 Backward ${i+1}: ${result.chapter} pages=[${result.pages}] scroll=${result.scrollLeft}`);
    });
    
    // Should have crossed chapter boundaries
    const allChapters = [...forwardResults, ...backwardResults].map(r => r.chapter);
    const uniqueChapters = [...new Set(allChapters)];
    expect(uniqueChapters.length).toBeGreaterThan(1);
    
    console.log(`✅ REMOTE Moby Dick: Forward(${forwardResults.length}) + Backward(${backwardResults.length}) = ${uniqueChapters.length} chapters`);
  });
});
...
test.describe('EPUB Navigation - Performance & Edge Cases', () => {
  ...
  test('SPEED TEST: Rapid navigation handling', async ({ page }) => {
    ...
    const rapidResults = await performBatchNavigation(page, 
      ['next', 'next', 'prev', 'next', 'prev', 'prev', 'next', 'next']
    );
    ...
    console.log(`⚡ SPEED: ${rapidResults.length} rapid navigations in ${duration}ms`);
  });
  ...
  test('BOUNDARY TEST: Navigation limits with both EPUBs', async ({ page }) => {
    ...
    // Go to beginning
    const beginningResults = await performBatchNavigation(page, ['prev', 'prev', 'prev', 'prev']);
    
    // Go to end
    const endResults = await performBatchNavigation(page, ['next', 'next', 'next', 'next', 'next', 'next']);
    ...
    console.log(`🎯 BOUNDARIES: Beginning(${beginningResults.length}) + End(${endResults.length}) states validated`);
  });
  
  test('CONTAINER TEST: Phantom element sizing across chapters', async ({ page }) => {
    await setupEpub(page, TEST_CONFIG.REMOTE_MOBY.url, true);
    ...
    containerData.forEach(data => {
      console.log(`📏 Container ${data.step}: ${data.chapter} width=${data.scrollWidth} scroll=${data.scrollLeft}`);
    });
    
    console.log(`✅ CONTAINER: ${containerData.length} steps, ${uniqueWidths.length} unique widths`);
  });
});
...
test.describe('EPUB Navigation - Cross-Format Validation', () => {
  ...
  test('COMPREHENSIVE: Local vs Remote EPUB behavior', async ({ page }) => {
    ...
    for (const config of testConfigs) {
      ...
      const chapterTransitions = navResults.filter((r, i) => 
        i > 0 && r.chapter !== navResults[i-1].chapter
      ).length;
    ...
    comparisonResults.forEach(result => {
      console.log(`${result.name}: ${result.validStates}/${result.totalNav} valid, ${result.chapterTransitions} transitions, local=${result.isLocal}`);
    });
    ...
    console.log('✅ CROSS-FORMAT: Both local and remote EPUBs behave consistently');
  });
});
